...
class MockNotificationService {
  private notifications: MockNotification[] = [
    {
      ...
      message: 'Your expense "Travel to Conference" for USD 150.00 has been approved by John Manager.',
      type: 'SUCCESS',
      read: false,
    ...
    },
    {
      ...
      message: 'Your expense "Office Supplies" for USD 75.00 has been submitted for approval.',
      type: 'INFO',
      read: false,
    ...
    },
    {
      ...
      message: 'Your expense "Business Lunch" for USD 45.00 has been rejected. Reason: Missing receipt attachment.',
      type: 'ERROR',
      read: true,
    ...
    },
    {
      ...
      message: 'Your expense "Hotel Booking" for USD 280.00 needs additional documentation.',
      type: 'WARNING',
      read: false,
  ...
  notifyExpenseApproval(expenseId: string, expenseTitle: string, amount: number, approverName: string): MockNotification {
    const notification: MockNotification = {
      id: Date.now().toString(),
      title: '✅ Expense Approved',
      message: `Your expense "${expenseTitle}" for USD ${amount.toFixed(2)} has been approved by ${approverName}.`,
      type: 'SUCCESS',
      read: false,
  ...
  notifyExpenseRejection(expenseId: string, expenseTitle: string, amount: number, reason: string): MockNotification {
    const notification: MockNotification = {
      id: Date.now().toString(),
      title: '❌ Expense Rejected',
      message: `Your expense "${expenseTitle}" for USD ${amount.toFixed(2)} has been rejected. Reason: ${reason}`,
      type: 'ERROR',
      read: false,
      createdAt: new Date().toISOString(),
      expenseId,
  ...
  notifyExpenseNeedsAttention(expenseId: string, expenseTitle: string, amount: number, reason: string): MockNotification {
    const notification: MockNotification = {
      id: Date.now().toString(),
      title: '⚠️ Expense Requires Attention',
      message: `Your expense "${expenseTitle}" for USD ${amount.toFixed(2)} needs attention. ${reason}`,
      type: 'WARNING',
      read: false,
      createdAt: new Date().toISOString(),
      expenseId,
  ...
  simulateNewNotification() {
    ...
    const mockApprovers = ['Sarah Johnson', 'Mike Wilson', 'Emma Davis', 'Alex Chen']
    ...
    if (notificationType < 0.6) {
      ...
      const randomReason = attentionReasons[Math.floor(Math.random() * attentionReasons.length)]
