...
async function checkConditionalApprovalRules(
  ...
  const approvedCount = approvalRequests.filter(
    (ar: any) => ar.status === "APPROVED"
  ).length;
  const rejectedCount = approvalRequests.filter(
    (ar: any) => ar.status === "REJECTED"
  ).length;
  ...
  if (currentRule.ruleType === "PERCENTAGE") {
    ...
    const maxPossiblePercentage =
      ((approvedCount + pendingCount) / totalApprovers) * 100;
  ...
  if (currentRule.ruleType === "SPECIFIC_APPROVER") {
    ...
    const specialApprovers = currentRule.approvers.filter(
    ...
    const hasSpecialApproval = approvalRequests.some(
      (ar: any) =>
        ar.status === "APPROVED" && specialApproverIds.includes(ar.approverId)
    );
  ...
  if (currentRule.ruleType === "HYBRID") {
    ...
    if (
      maxPossiblePercentage < requiredPercentage &&
      allSpecialApproversResponded
    ) {
      return { shouldApprove: false, shouldReject: true };
    }
